# Experiment Report: 20260323_091059_main_plus_topconf

## 实验背景
- 实验类型: 主实验+顶会Baseline
- 目标假设: H1, H2, H3
- 本轮问题: 统一训练设置下，现有方法与顶会可适配baseline的相对排序如何。
- 对应 idea.md: 三、核心研究问题, 六、方法方案, 七、实验设计

## 实验任务单
- run_id: `20260323_091059_main_plus_topconf`
- 成功标准: 输出可直接进入论文主表的统一对比结果，并识别最强可复现baseline。
- 失败标准: 若新增baseline训练不稳定或整体无增益，则回退到主实验配置。
- 数据集: /home/alexmhliu/affect-state-forecaster/data/processed/pheme_forecast_ratio_05_train.jsonl
- ratio: ratio_05_train
- 模型: affect_state_forecaster, patchtst_baseline, structure_baseline, temporal_baseline, text_baseline, thread_transformer_baseline, timesnet_baseline
- 关键参数: epochs=5, batch_size=16, device=cuda
- 特殊设置: baseline_family=main_plus_topconf

## 运行过程摘要
- 创建时间: 2026-03-23T01:12:02.031195+00:00
- Run root: `runs/20260323_091059_main_plus_topconf`
- 日志路径: `experiments/logs/20260323_091059_main_plus_topconf.log`
- 总耗时(秒): 0.0
- Warning 数: 9
- Error 数: 0
- 是否全部成功: 是
...
## 结果总览
- 最优模型: `affect_state_forecaster`，MAE=0.10519883036613464, RMSE=0.1745268702507019
- 与上一轮相比: 上一轮 `20260323_091019_topconf_baselines` 的最优模型是 `timesnet_baseline`，本轮最佳 MAE 变化 -0.0028。
- `affect_state_forecaster`: status=completed, MAE=0.10519883036613464, RMSE=0.1745268702507019, Pearson=-0.20580414210080983, Spearman=-0.07548558645422913
- `patchtst_baseline`: status=completed, MAE=0.10546157509088516, RMSE=0.1713934689760208, Pearson=-0.02700924838960847, Spearman=0.34987095132891766
...
- `thread_transformer_baseline`: status=completed, MAE=0.11836011707782745, RMSE=0.17733491957187653, Pearson=-0.11195826259729848, Spearman=0.32117207417416793
- `timesnet_baseline`: status=completed, MAE=0.13201002776622772, RMSE=0.181034117937088, Pearson=-0.06584551203541304, Spearman=0.1626697841725481

## 图表解读
- Model metrics: `experiments/figures/20260323_091059_main_plus_topconf_model_metrics.png`
  该图用于比较模型整体误差，当前 MAE 最优模型是 `affect_state_forecaster`。
- Event errors: `experiments/figures/20260323_091059_main_plus_topconf_event_errors.png`
  该图用于查看不同事件上的误差波动，帮助判断 cross-event 稳定性。
- Ratio trends: `experiments/figures/20260323_091059_main_plus_topconf_ratio_trends.png`
  当前只有一个 observation ratio，因此该图主要起到占位作用，尚不能判断趋势。

## 对 idea.md 的判断
### H1 早期观测足以预测未来群体情绪
- Verdict: 部分支持
- 最佳模型为 affect_state_forecaster，相对文本基线 MAE 改进 0.0472。
- 最佳模型相关系数表现 Pearson=-0.20580414210080983, Spearman=-0.07548558645422913。
### H2 Affect-State Forecaster 优于直接预测基线
- Verdict: 支持
- Affect-State Forecaster: MAE=0.10519883036613464, RMSE=0.1745268702507019。
